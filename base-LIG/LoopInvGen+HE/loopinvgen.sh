...
Configuration:
    ...
    [--expressiveness-level, -L <int>]  ($EXPRESSIVENESS_LEVEL)\t    {1 .. 6}
...
OPTS=`getopt -n 'parse-options' -o :P:R:I:V:cvL:l:p:s:S:t:z: --long Process-args:,Record-args:,Infer-args:,Verify-args:,clean-intermediates,verify,expressiveness-level:,log:,intermediates-dir:,states-to-record:,stats-path:,infer-timeout:,z3-path: -- "$@"`
if [ $? != 0 ]; then usage ; fi
...
while true ; do
  case "$1" in
    ...
    -L | --expressiveness-level )
         [ "$2" -ge "1" ] && [ "$2" -le "6" ] \
           || usage "Expressiveness level (= $2) must be between 1 and 6 (both inclusive)."
