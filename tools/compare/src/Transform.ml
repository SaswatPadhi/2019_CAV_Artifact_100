...
let spec =
  let open Command.Spec in (
    ...
    +> flag "-t" no_arg
       ~doc:"Translate a SyGuS-INV benchmark to SyGuS-General"
    +> flag "-r" no_arg
       ~doc:"Replace (Variable T) with list of variables"
    +> flag "-c" no_arg
       ~doc:"Replace (Constant T) with list of known constants"
    +> flag "-a" no_arg
       ~doc:"Binarize variadic versions of +, *, and, or"
    +> flag "-s" (optional string)
       ~doc:"FILENAME Sanitize var & func names and generate a mapping file"
    +> flag "-u" (optional string)
       ~doc:"FILENAME Undo sanitization using a given mapping file"
